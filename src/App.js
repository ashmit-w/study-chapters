...
function App() {
  ...
  useEffect(() => {
    const savedSubjects = localStorage.getItem('studyChapters_subjects');
    const savedNextId = localStorage.getItem('studyChapters_nextId');
    ...
    if (savedNextId) {
      setNextChapterId(parseInt(savedNextId));
    }
  }, []);

  // Save data to localStorage whenever subjects or nextChapterId changes
  useEffect(() => {
    localStorage.setItem('studyChapters_subjects', JSON.stringify(subjects));
  }, [subjects]);

  useEffect(() => {
    localStorage.setItem('studyChapters_nextId', nextChapterId.toString());
  }, [nextChapterId]);
  ...
  const clearAllData = () => {
    if (window.confirm('Are you sure you want to clear all data? This cannot be undone.')) {
      localStorage.removeItem('studyChapters_subjects');
      localStorage.removeItem('studyChapters_nextId');
